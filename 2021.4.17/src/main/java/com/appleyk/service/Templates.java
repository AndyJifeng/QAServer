...
//            templates.add(temp);
            temp.add(entity1+"-"+relation+"-"+entity2Map.get("labell")+"-"+key+"-查找实体");
...
//        System.out.println("Compare question:"+jsonToTemplate.OneEntityCompareTemplate(map).toString());
    ...
    public List<Object> addTemplates(List<Object> list,List<Object> templates){
        if(templates.size() == 0)
            return list;
        System.out.println(list);
        for (int i = 0; i < templates.size(); i++) {
            if(((List<Object>)(templates.get(i))).get(0).toString().equals((list).get(0).toString())){
                List<String> template = (List<String>) ((List<Object>)templates.get(i)).get(1);
                List<String> addTemplate = (List<String>)(list).get(1);
                for(String add : addTemplate){
